...
function checkRepeatSubmit(config) {
  if (config.method !== 'post' && config.method !== 'put') return null
  ...
  if (requestSize >= 5 * 1024 * 1024) {
    console.warn(`[${config.url}]: 请求数据大小超出5M限制，无法进行防重复提交验证。`)
    return null
  }
  ...
  if (sessionObj.data === requestObj.data && requestObj.time - sessionObj.time < interval) {
